...
const hasVowel = (str) =>
  !Array.from(str).every((char) => !/^[aeiou]$/.test(char.toLowerCase()));
const hasNumber = (str) =>
  !Array.from(str).every((char) => !/^[0123456789]$/.test(char.toLowerCase()));

const challenge7 = (str) =>
  new Promise((res) => {
    setTimeout(() => {
      if (hasVowel(str)) {
    ...
    .then((response) => {
      if (response.status === 10) {
        response.pass = response.status === 10 && hasVowel(response.value);
      } else if (response.status === 20) {
        response.pass = response.status === 20 && hasNumber(response.value);
    ...
    .catch((err) => {
      return {
        pass: !hasVowel(str) && !hasNumber(str) && err.message === "bad status",
        value: str,
        err,
      };
    });
...
setTimeout(() => {
  ...
  const challenge7Tests = Array.from(new Array(50), () =>
    challenge7(randomString())
  );
