...
const challenge6 = (str) =>
  new Promise((res) => {
    setTimeout(() => {
      res({
        status: hasVowel(str) ? 10 : 20,
        value: str,
      });
    }, 0);
  })
    ...
    .catch((err) => {
      return {
        pass: err.name === "Error" && err.message === "no vowels",
        value: str,
        err,
      };
    });
...
setTimeout(() => {
  ...
  const challenge6Tests = Array.from(new Array(50), () =>
    challenge6(randomString())
  );
